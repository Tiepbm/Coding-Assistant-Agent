...
description: 'Use when implementing API contracts: OpenAPI specs, GraphQL schemas, gRPC proto files, contract tests. Code-level patterns only — defer API trade-offs (versioning policy, public vs internal, breaking-change governance) to CE7.'
---
# API Design Pack (Implementation Patterns)

> **Scope boundary:** This pack covers *how to write* API contracts and code, NOT whether to choose REST vs GraphQL, public-facing versioning policy, or backwards-compatibility governance — those are architecture decisions → defer to CE7 Software Engineering Agent.

## When to Use
- Writing or updating OpenAPI 3.1 spec for a REST endpoint.
- Defining GraphQL schema (SDL) and resolver implementation.
...
| `graphql-schema` | Designing GraphQL schemas, resolvers, DataLoader, code-first vs SDL-first. |
